...
public class TabbedMDI
	 ...
	 * @param props_prefix         Prefix for loading MDIs properties (open history, etc)<br/>
	 ...
	 * @implNote viewID and props_prefix would be the same value if it weren't for legacy plugin code.
	...
	private void addDragDropListeners() {
		...
		tabFolder.addListener(SWT.Dispose, (ev) -> {
			dragSource.dispose();
			dropTarget.dispose();
		});
	}
