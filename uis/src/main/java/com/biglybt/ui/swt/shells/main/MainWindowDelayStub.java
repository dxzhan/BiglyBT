	...
	UIFunctionsSWTImpl
		...
		addTorrentWithOptions(
			...
			return ((Boolean) fixup(new Fixup4() {
				@Override
				public Object fix(UIFunctionsSWT uif) {
					return (uif.addTorrentWithOptions(torrentOptions, options));
				}
			}));
		}
		...
		openTorrentOpenOptions(
			...
			fixup(new Fixup3() {
				@Override
				public void fix(UIFunctionsSWT uif) {
					uif.openTorrentOpenOptions(shell, sPathOfFilesToOpen, sFilesToOpen, options);
				}
			});
		}
